...
`verify-release.yaml` exposes the composite as a `workflow_dispatch` entry point that runs the same validation against an arbitrary `(sha, version)` pair from the Actions UI. It carries `contents: read` only — no App token, no write permissions, no tag side effects — and exists so the gate can be exercised end-to-end without merging a fake release PR or installing the release App on a scratch repo (which the App's single-installation invariant forbids).
